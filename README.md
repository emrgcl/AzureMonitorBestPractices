...
# Demo 1 - Insights about insights
...
# Demo 2 - Alert Best Practices - Create Actionable Alert
1. Create alert when a service stops using kusto, and create a service start powershell script using a runbook.
1. Show an example of an alert with executive ready description.
```
// ## SQL SERVICE ALERT - WINDOWS SERVER

ConfigurationChange
| where Computer in (grp_SQL)
| where ConfigChangeType == "WindowsServices"
| where SvcDisplayName == "SQL Server (MSSQLSERVER)"
...
let diskSize = materialize(InsightsMetrics
| where Computer in (grp_NEONATALE_PROD)
| where Name == "FreeSpaceMB"
| extend diskUnit = tostring(parse_json(Tags).["vm.azm.ms/mountId"])
| summarize arg_max(TimeGenerated, *) by Computer, diskUnit
| extend diskSizeMb = toreal(parse_json(Tags).["vm.azm.ms/diskSizeMB"])
| extend Free_MB = Val
| distinct TimeGenerated, Computer, diskUnit, diskSizeMb, Free_MB, _ResourceId);
let disk_free_space_percent = materialize(InsightsMetrics
| where Computer in (grp_NEONATALE_PROD)
| where Name == "FreeSpacePercentage"
| extend diskUnit = tostring(parse_json(Tags).["vm.azm.ms/mountId"])
| summarize (TimeGenerated, Perc_free_Space) = arg_max(TimeGenerated, Val) by Computer, diskUnit, _ResourceId
| project TimeGenerated, Computer, diskUnit, Perc_free_Space, _ResourceId);
disk_free_space_percent
| join (diskSize) on Computer, diskUnit
| extend percThreshold = case(
    diskSizeMb <= 51200, 10, //50Gb
    diskSizeMb <= 102400, 8, //100 Gb
    diskSizeMb <= 204800, 6, //200 Gb
    diskSizeMb <= 307200, 4, //300 Gb
    diskSizeMb <= 1024000, 2, //1Tb
    2)
| extend mbThreshold = case(
    diskSizeMb <= 51200, 200, //50Gb
    diskSizeMb <= 102400, 1024, //100 Gb
    diskSizeMb <= 204800, 2048, //200 Gb
    diskSizeMb <= 307200, 3072, //300 Gb
    diskSizeMb <= 1024000, 10240, //1Tb
    20480)
| extend percThreshold = iif((_ResourceId contains "microsoft.compute/virtualmachines") and (diskUnit == "D:"), 0, percThreshold)
| extend mbThreshold = iif((_ResourceId contains "microsoft.compute/virtualmachines") and (diskUnit == "D:"), 0, mbThreshold)
| where Perc_free_Space < percThreshold or Free_MB < mbThreshold
| project TimeGenerated, Computer, ["Unit"]=diskUnit, ["Disk Size MBytes"] = diskSizeMb, ["Alert Threshold %"] = percThreshold, ["Free disk space %"] = Perc_free_Space, ["Alert Threshold MBytes"] = mbThreshold, ["Free disk space MBytes"] = Free_MB
| sort by Computer asc, Unit asc
```
